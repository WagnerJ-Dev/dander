...
| Batteries-included infrastructure | Live-proven in the retained sandbox | The read-only verifier passes for the GCS backend, four BigQuery datasets, immutable Cloud Run jobs, Scheduler jobs, per-runtime IAM, billing-account viewer bindings, and the HubSpot Secret Manager container. Bootstrap tests and the all-paused manual workflow cover the clean-project path; a new-project apply remains separately approval-gated. |
...
| Approval-gated release proof | Implemented | The WIF workflow uses current additive CLI flags, forces both schedules paused, provisions secret containers before values, verifies both jobs, preserves proof files, and always records a non-deleting resource inventory. A fresh-project dispatch still requires explicit approval. |
