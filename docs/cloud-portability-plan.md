...
Baseline: `origin/main` at `9dc5562` (`0.6.0rc1`)
...
The implementation starts from a proven GCP slice, but the current names and types are not yet
portable.
...
6. provider-specific throughput, incremental, transform, concurrency, and cost benchmarks.

Exit gate:

- all four warehouse adapters satisfy the first-class definition and produce equal canonical
  results for the shared conformance dataset.
...
1. execute the approved benchmark matrix and publish normalized reports;
