...
| BigQuery Writer | Substantially implemented | Idempotent SCD1/incremental MERGE, append-only partitioned snapshots, transactional SCD2, sandbox replace, committed watermarks, target-node dispatch, and bounded truncate/append load jobs | Controlled target schema evolution and Storage Write API selection for streaming workloads |
...
1. Add controlled schema evolution and Storage Write API workload selection.
