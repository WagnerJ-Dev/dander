...
| BigQuery Writer | Substantially implemented | Idempotent SCD1/incremental MERGE, append-only partitioned snapshots, transactional SCD2, sandbox replace, committed watermarks, target-node dispatch, bounded load jobs, and opt-in additive scalar schema evolution | Storage Write API selection for streaming workloads |
...
1. Add Storage Write API workload selection.
